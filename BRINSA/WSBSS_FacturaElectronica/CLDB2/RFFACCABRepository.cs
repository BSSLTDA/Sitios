...
namespace CLDB2
{
    public class RFFACCABRepository : IRFFACCABRepository
    {
        ...
        public string UpdCab(string Prefijo, string Factura)
        {
            string res = "";
            ...
            sbF.Add(string.Format("        AND FFACTUR = {0}", Factura));
        ...
        public string UpdExistePDF(string Prefijo, string Factura)
        {
            string res = "";
            CreaSQL sbF = new CreaSQL();
            sbF.Ini(" UPDATE");
            sbF.Add("        RFFACCAB");
            sbF.Add(string.Format(" SET    FAREF02 = '{0}'", Prefijo + Factura.PadLeft(8, '0')));
            sbF.Add(" , FAFLAG04 = 1");
            sbF.Add(" WHERE");
            sbF.Add(string.Format("        FPREFIJ     = '{0}'", Prefijo));
            sbF.Add(string.Format("        AND FFACTUR = {0}", Factura));
